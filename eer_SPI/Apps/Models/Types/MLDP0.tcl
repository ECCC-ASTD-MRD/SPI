...
proc MLDP0::ExtractMetFiles { } {
   ...
   if { $simdur <= 0 } {
      puts stdout ""
      Debug::TraceProc "MLDP0: Error! Not enough available meteorological data files in database according to emission date-time."
      ...
      Debug::TraceProc "MLDP0:        - Last date-time (stamp)       : $lastdate ($laststamp)"
   ...
   if { $nbtrials == 0 && $nbprogs == 0 } {
      puts stdout ""
      Debug::TraceProc "MLDP0: Error! No diagnostics and prognostics meteorological files."
      Dialog::CreateError .mldp0new "[lindex $Error(MetFiles) $GDefs(Lang)]" $GDefs(Lang) 600
      return False
   }
...
proc MLDP0::SimInitNew { } {
   ...
   set Sim(Scale)                "FINE"                              ; #----- Grid resolution string.
      ...
      if { [llength $line] == 8 } {
         ...
         if { [llength $Tmp(Iso)] < $Sim(EmMaxIso) && [lsearchsub $Tmp(Iso) $name 0] == -1 } {
            ...
            if { $halflife >= 900 } {
               ...
               Dialog::CreateDefault .mldp0new 500 "[lindex $Lbl(Warning) $GDefs(Lang)]" "[lindex $Warning(HalfLife) $GDefs(Lang)] $name." warning 0 "OK"
...
proc MLDP0::SpeciesStart { } {
  ...
  if { $SPECIES(Open) == 0 } {
     ...
     fileevent $SPECIES(Job) readable "MLDP0::SpeciesFormat"
  } else {
...
#----------------------------------------------------------------------------
# Nom        : <MLDP0::ValidateEffectiveSimDuration>
