...
namespace eval NowCaster::Obs { } {
   ...
   set Param(Flags) {
      { "Observations au-dessus de la terre (masque terre/mer)" "Above groud observations (land/sea mask)" }
      { "Vent de surface utilisé (stations terrestres)" "Surface winds used (ground stations)" }
      { "Enregistrement contient des données sur la correction de radiation (stations aérologiques)" "Record contains radiative correction data (aerologic stations)" }
      { "Enregistrement contient correction de la position des bateaux, provenant du CQ des bateaux" "Record contains ship position correction, coming from ships QC" }
      { "Inconsistance détectée par un processus de CQ" "Inconsistency detected from QC process" }
      { "Station hors du domaine d'intérêt" "Station outside domain of interest" }
      { "Station avec élément rejeté par le CQ de AO" "Station with rejected elements from QCl of OA" }
      { "Station avec élément(s) sur liste noire" "Station with blacklisted elements" }
      { "Station à évaluer mais non utilisée par AO" "Statio to be evaluated but not used for OA" }
      { "Décodeur rapporte position de station douteuse" "Decoder reports dubious station position" }
      { "Enregistrement contient des données observées" "Record contains observed data" }
      { "Enregistrement contient des données dérivées d'entrée" "Record contains entry derivate data" }
      { "Enregistrement contient des données vues par l'AO" "Record contains data viewed from OA" }
      { "Enregistrement contient des résidus" "Record contains residual" }
      { "Partie RADAT de TEMP (ou) présence de Ts de SATOB (ou) trajectoire reelle des RAOBS" "Part RADAT ofe TEMP (or) presence of Ts from SATOB (or) reel trajectories of RAOBS" }
      { "Partie A de TEMP, PILOT, SATEM ou SATOB" "Part A of TEMP, PILOT, SATEM or SATOB" }
      { "Partie B de TEMP, PILOT ou SATEM" "Part B of TEMP, PILOT, SATEM" }
      { "Partie C de TEMP, PILOT ou SATEM" "Part C of TEMP, PILOT, SATEM" }
      { "Partie D de TEMP, PILOT ou SATEM" "Part D of TEMP, PILOT, SATEM" }
      { "Enregistrement contient des données analysées" "Record contains analysed data" }
      { "Enregistrement contient des données prévues" "Record contains forecast data" }
      { "Enregistrement contient des données de vérification" "Record contains verification data" }
      { "Données satellitaires reçues via réception locale - (RARS) (ou)lat/lon/elev proviennent du BUFR - (SYNOP,BOUEES,RAOBS)" "Satellite data received through local broadcast - (RARS) (or)lat/lon/elev coming from BUFR - (SYNOP,BOUEES,RAOBS)" } 
      { "Observations proviennent d'un format BUFR et non ASCII" "Observation coming from BUFR, not ASCII" } }
      
   set Param(MarkerOp)   AND
   set Param(Markers) {
      { 0   ADE   { "Élément modifié ou généré par l'ADE" "Element modified or generated by the ADE" } }
      { 1   ADE   { "Élément qui excède un extrême climatologique" "Element which exceeds a climatologic extreme" } }
      { 2   ADE   { "Élément erroné" "Wrong element" } }
      { 3   ADE   { "Élément peut-être erroné" "Might be wrong element" } }
      { 4   DERIV { "Élément douteux" "Doubtless element" } }
      { 5   DERIV { "Élément interpolé, généré par DERIVATE" "Interpolated element generated by the DERIVATE" } }
      { 6   DERIV { "Élément corrigé par DERIVATE ou correction de biais" "Corrected element by the DERIVATE or bias" } }
      { 7   DERIV { "Réservé" "Reserved" } }
      { 8   AO    { "Élément rejeté parce qu'il est sur une liste noire" "Rejected element by blacklist" } }
      { 9   AO    { "Élément rejeté par le CQ de l'AO (Background Check ou QC-Var)" "Rejected element by QC of OA (Background Check or QC-Var) " } }
      { 10  AO    { "Élément généré par l'AO" "Element generated by OA" } }
      { 11  AO    { "Élément rejeté par un processus de sélection (thinning ou canal)" "Rejected element by selection process (thinning or channel)" } }
      { 12  AO    { "Élément assimilé (c'est-à-dire ayant influencé l'analyse)" "Assimilated element (influenced anaysis)" } }
      { 13  AO    { "Comparaison contre le champ d'essai, niveau 1" "Comparison against trial field, level 1" } }
      { 14  AO    { "Comparaison contre le champ d'essai, niveau 2" "Comparison against trial field, level 2" } }
      { 15  AO    { "Comparaison contre le champ d'essai, niveau 3" "Comparison against trial field, level 3" } }
      { 16  AO    { "Élément rejeté par la comparaison contre le champ d'essai (Background Check)" "Rejected element by comparing against trial field (Background Check)" } }
      { 17  AO    { "Élément rejeté par le QC-Var" "Rejected element by QC-Var" } }
      { 18  DERIV { "Élément non-utilisé à cause de l'orographie" "Non used element because of orography" } }
      { 19  DERIV { "Élément non-utilisé à cause du masque terre-mer" "Non used element because of land-sea mask" } }
      { 20  DERIV { "Erreur de position d'avion décelée par TrackQc" "Plane position error found by TrackQc" } }
...
proc NowCaster::Obs::Window { Frame } {
   ...
   labelframe $Frame.elem -text [lindex $Lbl(Elem) $GDefs(Lang)]
      ...
      frame $Frame.flags.flag
         label $Frame.flags.flag.lbl -text "STN FLAG" -width 11 -anchor w
         entry $Frame.flags.flag.ent -bg $GDefs(ColorLight) -textvariable NowCaster::Obs::Data(Flag)
         menubutton $Frame.flags.flag.sel -menu $Frame.flags.flag.sel.menu -bitmap @$GDefs(Dir)/share/bitmap/down.xbm
         menu $Frame.flags.flag.sel.menu -tearoff 1
         set i 0
         foreach flag $Param(Flags) {
            set NowCaster::Obs::Data(Flag$i) 0
            $Frame.flags.flag.sel.menu add checkbutton -offvalue 0 -onvalue [expr 1<<$i] -label [format "%s" [lindex $flag $GDefs(Lang)]] \
               -variable NowCaster::Obs::Data(Flag$i) -command { NowCaster::Obs::UpdateFlags False }
            incr i
         }
         pack $Frame.flags.flag.lbl $Frame.flags.flag.sel -side left -fill y
         pack $Frame.flags.flag.ent -side left -fill x -expand True
      ...
      frame $Frame.flags.mark
         ...
         foreach marker $Param(Markers) {
            set i [lindex $marker 0]
            set NowCaster::Obs::Data(Marker$i) 0
            $Frame.flags.mark.sel.menu add checkbutton -offvalue 0 -onvalue [expr 1<<$i] -label [format "%-5s %s" [lindex $marker 1] [lindex [lindex $marker 2] $GDefs(Lang)]] \
               -variable NowCaster::Obs::Data(Marker$i) -command { NowCaster::Obs::UpdateFlags False }
         }
      ...
      pack $Frame.flags.flag $Frame.flags.codtyp $Frame.flags.bfam $Frame.flags.bktyp $Frame.flags.mark $Frame.flags.nval -side top -fill x -padx 2 -pady 2 -expand True

      bind $Frame.flags.bfam.ent <Return>   { NowCaster::Obs::UpdateFlags True }
      bind $Frame.flags.flag.ent <Return>   { NowCaster::Obs::UpdateFlags True }
...
proc NowCaster::Obs::UpdateFlags { Manual { Obs {} } } {
   ...
   if { $Manual } {
      ...
      set i 0
      foreach flag $Param(Flags) {
         set Data(Flag$i) [expr $Data(Flag)&$bit]
         set bit [expr $bit<<1]
         incr i
      }
      ...
      set i 0
      foreach flag $Param(Flags) {
         set Data(Flag) [expr $Data(Flag)|$Data(Flag$i)]
         incr i
      }
...
proc NowCaster::Obs::Update { { Obs {} } } {
   ...
   foreach obs $Obs {
      ...
      metobs define $obs -VALID $NowCaster::Data(Sec) False -PERSISTANCE $NowCaster::Data(Persistance) -LEVELS  $Data(Levels$obs) -FAMILY $Data(Family$obs) \
         -CODETYPE $Data(Code$obs) -FLAG $Data(Flag$obs) -MARKER $Data(Marker$obs) -MARKEROP $Param(MarkerOp) -TYPE $Data(Type$Obs) -NVAL $Data(NVal$Obs)
