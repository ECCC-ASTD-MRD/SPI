...
int TkglCanvPostscriptExpandedCmd(Tcl_Interp *interp) {
...
\n% strings -An array of strings, one for each line of the text item,in order from top to bottom.\
\n% spacing -Spacing between lines.\
\n% xoffset -Horizontal offset for text bbox relative to x and y: 0 for nw/w/sw anchor, -0.5 for n/center/s, and -1.0 for ne/e/se.\
\n% yoffset -Vertical offset for text bbox relative to x and y: 0 for nw/n/ne anchor, +0.5 for w/center/e, and +1.0 for sw/s/se.\
\n% justify -0 for left justification, 0.5 for center, 1 for right justify.\
\n% stipple -Boolean value indicating whether or not text is to be drawn in stippled fashion. If text is stippled,\
\n%          procedure StippleText must have been defined to call\
\n%          StippleFill in the right way.\
\n%\
\n% Also, when this procedure is invoked, the color and font must already have been set for the text.\
\n\
\n/DrawRotatedText {\
\n   /stipple exch def\
\n   /justify exch def\
...
\n} bind def",(char*)NULL);
    ...
    /* ARGSUSED */
int
TkglCanvPostscriptCmd(canvasPtr, interp, argc, argv)
