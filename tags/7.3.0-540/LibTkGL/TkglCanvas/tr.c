...
/*
 * To place a (sub) viewport.
 */
int trViewport(TRcontext *tr,int X,int Y,int Width,int Height) {

   if (!tr) {
      glViewport(X,Y,Width,Height);
   } else {
      X-=tr->CurrentColumn*tr->TileWidthNB-tr->TileBorder;
      Y-=tr->CurrentRow*tr->TileHeightNB-tr->TileBorder;
      glViewport(X,Y,Width,Height);
   }
   return(1);
}
 ...
 * To place a scissor.
 */
int trScissor(TRcontext *tr,int X,int Y,int Width,int Height) {
