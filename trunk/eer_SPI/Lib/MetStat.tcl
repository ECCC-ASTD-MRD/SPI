...
namespace eval MetStat { } {
   ...
   set Rec(TypeD) "Donnees brutes aux stations"
   set Rec(TypeE) "Erreur mensuelle"
   set Rec(TypeK) "Constantes variees"
   set Rec(TypeM) "Matrice de verification"
   set Rec(TypeO) "Observation"
   set Rec(TypeP) "Prevision"
   ...
   while { ![eof $file] } {
      ...
      if { [string first "DEFVAR" [string toupper $line]]!=-1 } {
         ...
         if { [catch {MetStat::RECRCEval $cmd}] } {
            Dialog::CreateError . "[lindex $Msg(RECRC) $GDefs(Lang)]\n\n$cmd" $GDefs(Lang)
         }
      } else {
         gets $file line
      }
