...
int Tkgeoeer_Init(Tcl_Interp *Interp) {
   ...
   if (GLRender->UseThreads && !GLRender->XBatch) {
      if (Tcl_CreateThread(&tid,GDB_ThreadProc,NULL,TCL_THREAD_STACK_DEFAULT,TCL_THREAD_NOFLAGS)==TCL_ERROR) {
         App_Log(APP_WARNING,"%s: Unable to initiate GDB thread\n",__func__);
      }
   }
   ...
   if ((proj=Projection_Get(vp->Projection))) {
      ...
      if (GLRender->Wait || GLRender->TRCon) {
         ...
         if ((clock()-sec)>=(vp->TimeOut*CLOCKS_PER_SEC)) {
            App_Log(APP_WARNING,"%s: Waited too long for data, rendering anyway\n",__func__);
         }
         vp->Update=TRUE;
      }
...
void ViewportSetup(Tk_Canvas Canvas,ViewportItem *VP,Projection *Proj,int Width,int Height,int Tile,int Clear,int PS){
   ...
   if ((VP->Width/Proj->LI)>(VP->Height/Proj->LJ)) {
      ...
      dl=Proj->LJ;
      if (Tile) {
         trOrtho(GLRender->TRCon,-VP->Cam->Aspect*as*dl,VP->Cam->Aspect*as*dl,-VP->Cam->Aspect*dl,VP->Cam->Aspect*dl,-VP->Cam->Aspect*3,z);
      } else {
         if (Proj->Perspective) {
            dl*=0.35;
            glFrustum(-VP->Cam->Aspect*as*dl,VP->Cam->Aspect*as*dl,-VP->Cam->Aspect*dl,VP->Cam->Aspect*dl,0.5,z);
         } else {
            glOrtho(-VP->Cam->Aspect*as*dl,VP->Cam->Aspect*as*dl,-VP->Cam->Aspect*dl,VP->Cam->Aspect*dl,-VP->Cam->Aspect*3,z);
         }
      }
      ...
      dl=Proj->LI;
      if (Tile) {
         trOrtho(GLRender->TRCon,-VP->Cam->Aspect*dl,VP->Cam->Aspect*dl,-VP->Cam->Aspect*as*dl,VP->Cam->Aspect*as*dl,-VP->Cam->Aspect*3,z);
      } else {
         if (Proj->Perspective) {
            dl*=0.35;
            glFrustum(-VP->Cam->Aspect*dl,VP->Cam->Aspect*dl,-VP->Cam->Aspect*as*dl,VP->Cam->Aspect*as*dl,0.5,z);
         } else {
            glOrtho(-VP->Cam->Aspect*dl,VP->Cam->Aspect*dl,-VP->Cam->Aspect*as*dl,VP->Cam->Aspect*as*dl,-VP->Cam->Aspect*3,z);
         }
      }
   }
...
static int ViewportToPostscript(Tcl_Interp *Interp,Tk_Canvas Canvas,Tk_Item *Item,int Prepass){
   ...
   while (((clock()-sec)<(vp->TimeOut*CLOCKS_PER_SEC)) && (!GDB_ThreadQueueIsEmpty(0x0) || (vp->Loading+proj->Loading)));
   if ((clock()-sec)>=(vp->TimeOut*CLOCKS_PER_SEC)) {
      App_Log(APP_WARNING,"%s: Warning, waited too long for data, rendering anyway\n",__func__);
   }
