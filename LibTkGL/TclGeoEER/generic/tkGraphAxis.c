 ...
 *---------------------------------------------------------------------------------------------------------------
*/
int GraphAxis_Layout(TGraphAxis *Axis,int Side,int Width,int Height,int *X0,int *Y0) {

   double dx,dy,dxl,dyl,s,c;
   ...
   s = sin(DEG2RAD(Axis->Angle));
   c = cos(DEG2RAD(Axis->Angle));
   *X0 = dx*c + dy*s+dxl;
   *Y0 = dy*c - dx*s+dyl;
