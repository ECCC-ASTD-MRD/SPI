...
static const Tk_ConfigSpec GraphSpecs[] = {
   { TK_CONFIG_ANCHOR, "-anchor", NULL, NULL, "nw",Tk_Offset(GraphItem,anchor),TK_CONFIG_DONT_SET_DEFAULT },
...
static void GraphDisplay(Tk_Canvas Canvas,Tk_Item *Item,Display *Disp,Drawable Draw,int X,int Y,int Width,int Height){
   ...
   if (!gr->Update && gr->Frame && !GLRender->TRCon && (!gr->wi || (gr->xi>=0 && gr->yi>=0 && (gr->xi+gr->wi)<(gr->xg[1]-gr->xg[0]) && (gr->yi+gr->hi)<(gr->yg[0]-gr->yg[1])))) {
