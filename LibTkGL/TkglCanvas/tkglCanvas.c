 ...
 *
 * RCS: @(#) $Id: TkglCanvas.c,v 1.14.2.2 2001/04/06 01:10:17 hobbs Exp $
 */
 ...
 *    glCanvasWidgetCmd, FindItems, glRelinkItems
 *   The only instances of the structure are owned by:
 *    glCanvasWidgetCmd
 *   glCanvasWidgetCmd is the only function that calls:
 *    FindItems, glRelinkItems
 *   glCanvasWidgetCmd, FindItems, glRelinkItems, are the only functions that call
 *    TagSearch*
 * )
 */
...
static Tk_ItemType *gltypeList = NULL; /* NULL means initialization hasn't been done yet. */

#ifndef USE_OLD_TAG_SEARCH
/*
 * Uids for operands in compiled advanced tag search expressions
 * Initialization is done by InitglCanvas()
 */
...
extern Tk_ItemType tkglBitmapType, tkglLineType,tkglPolygonType,tkglRectangleType,tkglOvalType,tkglArcType;
extern Tk_ItemType tkglImageType,tkglTextType,tkWindowType;

/* Prototypes for procedures defined later in this file */

static void    CanvasBindProc _ANSI_ARGS_((ClientData clientData,
             XEvent *eventPtr));
static void    glCanvasBlinkProc _ANSI_ARGS_((ClientData clientData));
static void    CanvasCmdDeletedProc _ANSI_ARGS_((
             ClientData clientData));
static void    CanvasDoEvent _ANSI_ARGS_((TkCanvas *canvasPtr,
             XEvent *eventPtr));
static void    glCanvasEventProc _ANSI_ARGS_((ClientData clientData,
             XEvent *eventPtr));
...
static Tk_Item *  CanvasFindClosest _ANSI_ARGS_((TkCanvas *canvasPtr,
             double coords[2]));
static void    glCanvasFocusProc _ANSI_ARGS_((TkCanvas *canvasPtr,
             int gotFocus));
static void    glCanvasLostSelection _ANSI_ARGS_((
             ClientData clientData));
static void    glCanvasSelectTo _ANSI_ARGS_((TkCanvas *canvasPtr,
             Tk_Item *itemPtr, int index));
static void    glCanvasSetOrigin _ANSI_ARGS_((TkCanvas *canvasPtr,
             int xOrigin, int yOrigin));
static void    CanvasUpdateScrollbars _ANSI_ARGS_((
             TkCanvas *canvasPtr));
static int     glCanvasWidgetCmd _ANSI_ARGS_((ClientData clientData,
             Tcl_Interp *interp, int argc, Tcl_Obj *CONST *argv));
static void    glCanvasWorldChanged _ANSI_ARGS_((
             ClientData instanceData));
static int     ConfigureglCanvas _ANSI_ARGS_((Tcl_Interp *interp,
             TkglCanvas *canvasPtr, int argc, Tcl_Obj *CONST *argv,
             int flags));
static void    DestroyglCanvas _ANSI_ARGS_((char *memPtr));
static void    DisplayglCanvas _ANSI_ARGS_((ClientData clientData));
static void             DisplayglCanvasItems _ANSI_ARGS_((ClientData clientData,int X,int Y,int Width,int Height));
static void    DoItem _ANSI_ARGS_((Tcl_Interp *interp,
             Tk_Item *itemPtr, Tk_Uid tag));
static void    EventuallyRedrawglItem _ANSI_ARGS_((Tk_Canvas canvas,
             Tk_Item *itemPtr));
#ifdef USE_OLD_TAG_SEARCH
...
static Tk_ClassProcs glcanvasClass = {
    sizeof(Tk_ClassProcs), /* size */
    glCanvasWorldChanged,  /* geometryProc. */
};
...
int
Tk_glCanvasObjCmd(clientData, interp, argc, argv)
    ...
    Tcl_Obj *CONST argv[];    /* Argument objects. */
{
   ...
   /* Initialize the TkglCanvas structure */
   canvasPtr = (TkglCanvas *) ckalloc(sizeof(TkglCanvas));
   canvasPtr->tkwin = new;
   canvasPtr->display = Tk_Display(new);
   canvasPtr->interp = interp;
   canvasPtr->widgetCmd = Tcl_CreateObjCommand(interp,
   Tk_PathName(canvasPtr->tkwin), glCanvasWidgetCmd,(ClientData) canvasPtr, CanvasCmdDeletedProc);
   ...
   Tk_SetClassProcs(canvasPtr->tkwin, &glcanvasClass, (ClientData) canvasPtr);
   Tk_CreateEventHandler(canvasPtr->tkwin,
      ExposureMask|StructureNotifyMask|FocusChangeMask,
      glCanvasEventProc, (ClientData) canvasPtr);
   ...
   if (ConfigureglCanvas(interp, canvasPtr, argc-2, argv+2, 0) != TCL_OK) {
...
static int
glCanvasWidgetCmd(clientData, interp, argc, argv)
    ...
    Tcl_Obj *CONST argv[];    /* Argument objects. */
{
   ...
   if (argc == 2) {
       ...
       result = ConfigureglCanvas(interp,canvasPtr, argc-2, argv+2,
          TK_CONFIG_ARGV_ONLY);
   }
   break;
      }
   ...
   for (typePtr = gltypeList; typePtr != NULL; typePtr = typePtr->nextPtr) {
       if ((c == typePtr->name[0])
...
#ifdef USE_OLD_TAG_SEARCH
   glRelinkItems(canvasPtr, argv[2], itemPtr);
#else /* USE_OLD_TAG_SEARCH */
        if ((result = glRelinkItems(canvasPtr, argv[2], itemPtr, &searchPtr)) != TCL_OK) {
            goto done;
        }
#endif /* USE_OLD_TAG_SEARCH */
   break;
...
#ifdef USE_OLD_TAG_SEARCH
   glRelinkItems(canvasPtr, argv[2], prevPtr);
#else /* USE_OLD_TAG_SEARCH */
        result = glRelinkItems(canvasPtr, argv[2], prevPtr, &searchPtr);
        if (result != TCL_OK) {
            goto done;
        }
#endif /* USE_OLD_TAG_SEARCH */
   ...
   if (!index) {
       ...
       glCanvasSetOrigin(canvasPtr, newXOrigin, newYOrigin);
   }
   break;
      }
      case CANV_SELECT: {
   ...
   switch ((enum options) optionindex) {
     case CANV_ADJUST: {
      ...
      if (index < (canvasPtr->textInfo.selectFirst
       ...
       glCanvasSelectTo(canvasPtr, itemPtr, index);
       break;
     }
     case CANV_CLEAR: {
       if (argc != 3) {
      ...
      EventuallyRedrawglItem((Tk_Canvas) canvasPtr,
...
static void DestroyglCanvas(char *memPtr){
   ...
   if (canvasPtr->flags & REDRAW_PENDING) {
      Tcl_CancelIdleCall(DisplayglCanvas,(ClientData)canvasPtr);
   }

   /* Free up all of the items in the canvas */

   for (itemPtr=canvasPtr->firstItemPtr;itemPtr!=NULL;itemPtr=canvasPtr->firstItemPtr) {
      canvasPtr->firstItemPtr = itemPtr->nextPtr;
      (*itemPtr->typePtr->deleteProc)((Tk_Canvas)canvasPtr,itemPtr,canvasPtr->display);
      if (itemPtr->tagPtr!=itemPtr->staticTagSpace) {
         ckfree((char*)itemPtr->tagPtr);
      }
      ckfree((char*)itemPtr);
...
static int
ConfigureglCanvas(interp, canvasPtr, argc, argv, flags)
   Tcl_Interp *interp;     /* Used for error reporting. */
   TkglCanvas *canvasPtr;  /* Information about widget;  may or may
    ...
    int flags;             /* Flags to pass to Tk_ConfigureWidget. */
{
   ...
   Tk_GeometryRequest(canvasPtr->tkwin, canvasPtr->width + 2*canvasPtr->inset,canvasPtr->height + 2*canvasPtr->inset);

    /* Restart the cursor timing sequence in case the on-time or off-time just changed */

   if (canvasPtr->textInfo.gotFocus)
      glCanvasFocusProc(canvasPtr, 1);
   ...
   canvasPtr->tsoffset.yoffset = canvasPtr->height;
    ...
    glCanvasSetOrigin(canvasPtr, canvasPtr->xOrigin, canvasPtr->yOrigin);
...
static void glCanvasWorldChanged(instanceData)
 ...
 *----------------------------------------------------------------------------
*/
static void DisplayglCanvasItems(ClientData clientData,int X,int Y,int Width,int Height) {
   ...
   while (canvasPtr->flags & REPICK_NEEDED) {
      Tcl_Preserve((ClientData) canvasPtr);
      canvasPtr->flags &= ~REPICK_NEEDED;
      glPickCurrentItem(canvasPtr, &canvasPtr->pickEvent);
      Tcl_Release((ClientData) canvasPtr);
      if (!tkwin)
         return;
    }
...
static void glCanvasEventProc(clientData, eventPtr)
             ...
             * Pixels on edge are not redrawn. */
{
     ...
     * redraw it.
     */
    if (!canvasPtr || canvasPtr->tkwin == NULL) {
   return;
    }
...
static void
EventuallyRedrawglItem(canvas, itemPtr)
    Tk_Canvas canvas;      /* Information about widget. */
                ...
                * allocated (must live forever). */
{
   ...
   InitglCanvas();
    ...
    for (typePtr2 = gltypeList, prevPtr = NULL; typePtr2 != NULL;
       prevPtr = typePtr2, typePtr2 = typePtr2->nextPtr) {
   if (strcmp(typePtr2->name, typePtr->name) == 0) {
       if (prevPtr == NULL) {
      gltypeList = typePtr2->nextPtr;
       } else {
...
FindItems(interp, canvasPtr, argc, argv, newTag, first, searchPtrPtr)
    ...
    int first;          /* For error messages:  gives number
                ...
                * handled. */
#ifndef USE_OLD_TAG_SEARCH
    TagSearch **searchPtrPtr;           /* From glCanvasWidgetCmd local vars*/
#endif /* not USE_OLD_TAG_SEARCH */
{
#ifdef USE_OLD_TAG_SEARCH
    TagSearch search;
...
#ifdef USE_OLD_TAG_SEARCH
static void
glRelinkItems(canvasPtr, tag, prevPtr)
#else /* USE_OLD_TAG_SEARCH */
static int
glRelinkItems(canvasPtr, tag, prevPtr, searchPtrPtr)
#endif /* USE_OLD_TAG_SEARCH */
                ...
                * happened. */
{
   ...
   if (eventPtr->type == ButtonPress) {
       ...
       glPickCurrentItem(canvasPtr, eventPtr);
                ...
                * MotionNotify. */
{
   ...
   EventuallyRedrawglItem((Tk_Canvas) canvasPtr, prevItemPtr);
    ...
    ClientData clientData; /* Pointer to record describing entry. */
{
   ...
   canvasPtr->insertBlinkHandler = Tcl_CreateTimerHandler(
      canvasPtr->insertOffTime, glCanvasBlinkProc,
      (ClientData) canvasPtr);
    } else {
   canvasPtr->textInfo.cursorOn = 1;
   canvasPtr->insertBlinkHandler = Tcl_CreateTimerHandler(
      canvasPtr->insertOnTime, glCanvasBlinkProc,
      (ClientData) canvasPtr);
    }
...
static void glCanvasFocusProc(canvasPtr, gotFocus)
...
static void glCanvasSelectTo(canvasPtr, itemPtr, index)
             ...
             * "other" end of the selection. */
{
    ...
    if (canvasPtr->textInfo.selItemPtr == NULL) {
   Tk_OwnSelection(canvasPtr->tkwin, XA_PRIMARY, glCanvasLostSelection,
      (ClientData) canvasPtr);
...
static void glCanvasLostSelection(clientData)
...
static void glCanvasSetOrigin(canvasPtr, xOrigin, yOrigin)
    TkCanvas *canvasPtr;   /* Information about canvas. */
