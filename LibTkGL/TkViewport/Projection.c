...
void Murphy_WideLine(Vect3d P0,Vect3d P1,int Width) {
   ...
   tk = (int)(4.0 * hypot(pt[0]-p0[0], pt[1]-p0[1]) * hypot(m.u,m.v));  /* used here for constant thickness line */

   for (q=0;dd<=tk;q++) {    /* outer loop, stepping perpendicular to line */

      Murphy_ParaLine(&m,pt,d1);        /* call to inner loop - right edge */
...
void Murphy_ParaLine(TMurphy *M,Vect2i Pt,int D1) {
...
//      M->Plot(murphy.pb, &pt, murphy.color);
