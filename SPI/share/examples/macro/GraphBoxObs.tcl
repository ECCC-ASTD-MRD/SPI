...
namespace eval Macro::GraphBoxObs {
   variable Params
...
proc Macro::GraphBoxObs::Init { } {
   ...
   vector create DATA
   vector dim    DATA { X Y ERROR HIGH LOW MEDIAN MIN MAX }

   graphitem create ITEM
   graphitem configure ITEM -xaxis axisx -yaxis axisy \
      -xdata DATA.X -ydata DATA.Y -error DATA.ERROR -high DATA.HIGH -low DATA.LOW -median DATA.MEDIAN -min DATA.MIN -max DATA.MAX \
      -orient X -desc "bla bla" -iconoutline blue -iconfill green \
      -outline red -fill yellow -transparency 100 -width 2 -size 2 -value False \
      -type BOXPLOT -font XFont12 -icon CIRCLE -iconoutline red -iconfill black -bitmap "" -stipple "" -image ""
}

proc Macro::GraphBoxObs::Execute { } {
   variable Param

   #----- Initialize page and graph
   Macro::GraphBoxObs::Init
   ...
   for { set n 0 } { $n<[observation define [lindex $obs 0] -NB] } { incr n } {
      ...
      vector set DATA.X {}
      vector set DATA.Y {}
      vector set DATA.ERROR {}
      vector set DATA.HIGH {}
      vector set DATA.LOW {}
      vector set DATA.MEDIAN {}
      vector set DATA.MIN {}
      vector set DATA.MAX {}
      set dates {}
      ...
      foreach ob $obs {
         ...
         #----- Build data vectors (hypotetical values for box parameters)
         vector append DATA [list $sec $val [expr $val*1.05] [expr $val+1] [expr $val-1] $val [expr $val-10] [expr $val+10]]
      ...
      graphaxis configure axisx -type LINEAR -min [vector stats DATA.X -min] -max [vector stats DATA.X -max] -intervals [lsort -increasing [vector get DATA.X]] -labels $dates \
         -unit "Temps\n ttt"
      graphaxis configure axisy -type LINEAR -min 0 -max [vector stats DATA.Y -max] -unit Pbb -increment 10
...
#      PrintBox::Image $Frame png /tmp/$file
      break
   }
}
